"""Tests for gateway-policy rules and the trigger_takeover tool."""
...
class TestTakeover:
    ...
    def test_inactive_chat_passes_through(self, fresh_state, src_dm, session_store, gateway):
        ...
        only via the trigger_takeover tool, never from inbound text."""
        from gateway_policy.rules.takeover import takeover_rule
        event = FakeEvent(text="please let me speak to a human now", source=src_dm)
        result = takeover_rule(
            event=event, gateway=gateway, session_store=session_store, state=fresh_state
        )
        assert result is None
        assert not fresh_state.takeovers.is_active("whatsapp", src_dm.chat_id)
        assert len(session_store.appended) == 0
    ...
    def test_active_handover_silent_ingests_subsequent_messages(
        ...
        assert result == {"action": "skip", "reason": "takeover_active"}
    ...
    def test_other_platform_skipped(self, fresh_state, session_store, gateway):
        ...
        assert not fresh_state.takeovers.is_active("telegram", "-100")
    ...
    def test_owner_exit_command_in_active_chat_deactivates(
        ...
        event = FakeEvent(text="/handover", source=owner_src)
        result = takeover_rule(
            event=event, gateway=gateway, session_store=session_store, state=fresh_state
        )
        assert result == {"action": "skip", "reason": "takeover_exit"}
        assert not fresh_state.takeovers.is_active("whatsapp", src_dm.chat_id)
    ...
    def test_takeback_without_owner_flag_from_stranger_does_not_deactivate(
        ...
        assert result == {"action": "skip", "reason": "takeover_active"}
        assert fresh_state.takeovers.is_active("whatsapp", src_dm.chat_id)
...
class TestWhatsappAliasKeying:
    ...
    def test_lid_inbound_finds_phone_keyed_handover(
        ...
        # trigger_takeover call when the bridge reported phone form).
        fresh_state.takeovers.activate(
            "whatsapp",
    ...
    def test_owner_takeback_on_alias_form_deactivates(
        ...
        assert fresh_state.takeovers.get(
    ...
    def test_owner_extend_touches_existing_alias_row(
        ...
        before = fresh_state.takeovers.get("whatsapp", "60173380115@s.whatsapp.net")
        ...
        assert result == {"action": "skip", "reason": "takeover_owner_extend"}

        after = fresh_state.takeovers.get("whatsapp", "60173380115@s.whatsapp.net")
        assert after is not None
        assert after.activated_by == "trigger_takeover_tool"
    ...
    def test_trigger_takeover_reuses_existing_alias_row(
        self, fresh_state, gateway, monkeypatch
    ):
        """The agent-driven trigger_takeover tool must also detect an
        ...
        import json
        from gateway_policy.tools.trigger_takeover import make_trigger_takeover_tool
        ...
        _, handler = make_trigger_takeover_tool(lambda: fresh_state)
        ...
        assert len(fresh_state.takeovers.list_active()) == 1
...
class TestTouch:
    def test_touch_on_cold_row_is_noop(self, fresh_state, src_dm):
        ...
        ok = fresh_state.takeovers.touch("whatsapp", src_dm.chat_id, ttl_seconds=600)
        assert ok is False
        assert fresh_state.takeovers.get("whatsapp", src_dm.chat_id) is None
    ...
    def test_touch_on_hot_row_pushes_expires_at_forward(self, fresh_state, src_dm):
        ...
        original = fresh_state.takeovers.get("whatsapp", src_dm.chat_id)
        assert original is not None and original.expires_at is not None

        ok = fresh_state.takeovers.touch("whatsapp", src_dm.chat_id, ttl_seconds=600)
        assert ok is True
        bumped = fresh_state.takeovers.get("whatsapp", src_dm.chat_id)
    ...
    def test_touch_on_no_ttl_row_is_noop(self, fresh_state, src_dm):
        ...
        ok = fresh_state.takeovers.touch("whatsapp", src_dm.chat_id, ttl_seconds=600)
        assert ok is False
        row = fresh_state.takeovers.get("whatsapp", src_dm.chat_id)
...
class TestOwnerImplicit:
    ...
    def test_owner_inbound_on_cold_chat_activates_without_notify(
        ...
        assert result == {"action": "skip", "reason": "takeover_owner_activate"}
    ...
    def test_owner_inbound_on_hot_chat_slides_ttl_no_double_activate(
        ...
        before = fresh_state.takeovers.get("whatsapp", src_dm.chat_id)
        ...
        after = fresh_state.takeovers.get("whatsapp", src_dm.chat_id)
        assert after is not None
...
# ---------------------------------------------------------------------------
# trigger_takeover tool
# ---------------------------------------------------------------------------

class TestTriggerTakeoverTool:
    def test_activates_via_session_key_lookup(self, fresh_state, src_dm, gateway):
        ...
        schema, handler = make_trigger_takeover_tool(lambda: fresh_state)
        result_json = handler(
    ...
    def test_disabled_handover_rejects_tool_call(self, fresh_state):
        ...
        assert result["ok"] is False
        assert result["error_code"] == "takeover_disabled"
...
# ---------------------------------------------------------------------------
# trigger_takeover owner notification (hardcoded template in notify.py)
# ---------------------------------------------------------------------------

class TestNotifyTokens:
    def test_activate_message_uses_hardcoded_template_with_both_slash_lines(
        self, fresh_state, src_dm, gateway, monkeypatch
    ):
        """End-to-end: handler uses format_notify_on_activate (phone, link, both commands)."""
        import sys
        import types
        ...
        assert "Takeover:" in message
        assert "/takeover_122299244130458_AT_lid" in message
        assert "/handover_122299244130458_AT_lid" in message
...
class TestTelegramOwnerCommands:
    def test_owner_takeback_active_deactivates(
        ...
        assert adapter.sent
        assert "Handover complete" in adapter.sent[-1][1]
        assert any(
            isinstance(msg.get("content"), str)
            and msg["content"].startswith("[takeover-ended]")
    ...
    def test_owner_takeback_no_handover_warns(
        ...
        assert result == {"action": "skip", "reason": "owner_telegram_command"}
        adapter = gateway_wa_tg.adapters[Platform("telegram")]
        assert "No active takeover" in adapter.sent[-1][1]

    def test_owner_takeover_activate_hardcoded_notify(
        self, fresh_state, src_dm, session_store, gateway_wa_tg
    ):
        from gateway.config import Platform
        from gateway_policy.rules.telegram_owner_commands import (
        ...
        tok = encode_chat_id(src_dm.chat_id)
        event = FakeEvent(text=f"/takeover_{tok}", source=owner_tg)
        result = telegram_owner_commands_rule(
            event=event,
            gateway=gateway_wa_tg,
            session_store=session_store,
        ...
        assert tg.sent
        msg = tg.sent[-1][1]
        assert "Takeover:" in msg and "/takeover_" in msg and "/handover_" in msg
    ...
    def test_non_owner_telegram_does_not_skip(
        ...
        tok = encode_chat_id(src_dm.chat_id)
        event = FakeEvent(text=f"/handover_{tok}", source=stranger)
        assert (
            telegram_owner_commands_rule(
                event=event,
                gateway=gateway_wa_tg,
    ...
    def test_whatsapp_inbound_encoded_takeback_not_telegram_layer(
        ...
        tok = encode_chat_id(src_dm.chat_id)
        event = FakeEvent(text=f"/handover_{tok}", source=src_dm)
        assert (
            telegram_owner_commands_rule(
                event=event,
                gateway=gateway,
...
# ---------------------------------------------------------------------------
# boundary notes on takeover deactivation
# ---------------------------------------------------------------------------

class TestTakeoverBoundaryNote:
    """Confirm a `[takeover-ended]` marker is appended to the transcript on
    ...
    def test_lazy_expiry_writes_boundary_note_on_next_inbound(
        ...
        with fresh_state.takeovers._lock:
            fresh_state.takeovers._connect().execute(
    ...
    def test_expire_stale_returns_deleted_rows_and_no_others(
        ...
        """TakeoverStore.expire_stale only touches rows in `candidates` and
        returns the actually-deleted rows so the caller can react to the
        transition."""
        store = fresh_state.takeovers
...
class TestTakeoverStoreConnection:
    def test_sqlite_pragmas_busy_timeout_and_wal(self, tmp_path):
        from gateway_policy.state import TakeoverStore

        store = TakeoverStore(tmp_path / "conn_test.db")
        with store._lock:
            conn = store._connect()
